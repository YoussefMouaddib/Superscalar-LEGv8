...
module dispatch #(
    ...
    always_ff @(posedge clk or posedge reset) begin
        if (reset || flush_pipeline) begin
            ...
            for (int i = 0; i < FETCH_W; i++) begin
                if (rename_valid_r[i] &&
                    !(rename_is_alu_r[i] && rename_prd_r[i] == 6'd0)&& !(rename_prd_r[i] == 6'd0 && rename_psrc1_r[i] < 6'd30)) begin
                    rs_alloc_en[i] <= 1'b1;
            ...
            // ====================================================
            for (int i = 0; i < FETCH_W; i++) begin
                if (rename_valid_r[i] && !(rename_is_alu_r[i] && rename_prd_r[i] == 6'd0) && !(rename_prd_r[i] == 6'd0 && rename_psrc1_r[i] < 6'd30)) begin
                    rob_alloc_en[i] <= 1'b1;
            ...
            lsu_alloc_en <= 1'b0;
            for (int i = 0; i < FETCH_W; i++) begin
                if (rename_valid_r[i] && (rename_is_load_r[i] || rename_is_store_r[i])&& !(rename_prd_r[i] == 6'd0 && rename_psrc1_r[i] < 6'd30)) begin
                    lsu_alloc_en <= 1'b1;
                    lsu_lane_index <= i[0];
