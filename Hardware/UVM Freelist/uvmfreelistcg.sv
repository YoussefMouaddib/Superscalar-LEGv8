...
class freelist_monitor extends uvm_monitor;
  ...
  // coverage
  freelist_seq_item cov_tr;

  covergroup freelist_cg;

    // which allocation port combinations are being requested
    cp_alloc_en: coverpoint cov_tr.alloc_en {
      bins none        = {2'b00};
      bins port0_only  = {2'b01};
      bins port1_only  = {2'b10};
      bins both_ports  = {2'b11};
    }

    // which free port combinations are active
    cp_free_en: coverpoint cov_tr.free_en {
      bins none        = {2'b00};
      bins port0_only  = {2'b01};
      bins port1_only  = {2'b10};
      bins both_ports  = {2'b11};
    ...
    // DUT's response — did both ports come back valid
    cp_alloc_valid: coverpoint cov_tr.alloc_valid {
      bins none        = {2'b00};
      bins port0_only  = {2'b01};
      bins port1_only  = {2'b10};
    ...
    // free and alloc happening in the same cycle — the interesting timing case
    cp_free_and_alloc: coverpoint (cov_tr.free_en != 2'b00 && cov_tr.alloc_en != 2'b00) {
      bins inactive = {1'b0};
      bins active   = {1'b1};
    }

    // list exhaustion — alloc requested but nothing came back valid
    cp_alloc_starved: coverpoint (cov_tr.alloc_en != 2'b00 && cov_tr.alloc_valid == 2'b00) {
      bins not_starved = {1'b0};
      bins starved     = {1'b1};
    }

    // cross: which alloc combination was requested vs what actually came back
    // this is the most valuable one — catches cases where the DUT gives valid
    // on a port that wasn't requested, or fails to give valid when it should
    cx_req_vs_response: cross cp_alloc_en, cp_alloc_valid;

    // cross: simultaneous free and alloc, broken down by which ports
    cx_free_alloc_combo: cross cp_free_en, cp_alloc_en;
  ...
  function new(string name, uvm_component parent);
    super.new(name, parent);
    ap = new("ap", this);
    freelist_cg = new();    // covergroup must be constructed in new()
  endfunction
  ...
  task run_phase(uvm_phase phase);
    forever begin
      ...
      cov_tr = tr;       // update the handle the covergroup samples from
      freelist_cg.sample();   // trigger sampling explicitly
      ap.write(tr);
    end
  endtask
...
class freelist_base_test extends uvm_test;
  ...
  function void report_phase(uvm_phase phase);
    ...
    env.agt.mon.freelist_cg.get_coverage()), UVM_LOW)
  endfunction
