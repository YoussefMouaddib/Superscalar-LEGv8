...
module inst_rom #(
    ...
    always_ff @(posedge clk) begin
        ...
        // UART Hello World Program
            ...
            // Replace LSL X1, X1, #16 with two instructions:
            rom['h01] <= 32'h20A00010;  // ADDI X5, X0, #16  (X5 = shift amount)
            rom['h02] <= 32'h00010425;  // LSL  X1, X1, X5   (register shift: op=000000, rd=X1, rs1=X1, rs2=X5, func=000000)
            
            rom['h03] <= 32'h20410004;  // ADDI X2, X1, #4
            rom['h04] <= 32'h20A00001;  // ADDI X5, X0, #1
            rom['h05] <= 32'h21200002;  // ADDI X9, X0, #2
            
            // Replace LSL X9, X9, #12 with:
            rom['h06] <= 32'h20C0000C;  // ADDI X6, X0, #12  (X6 = shift amount)
            rom['h07] <= 32'h00112906;  // LSL  X9, X9, X6   (register shift: rd=X9, rs1=X9, rs2=X6)
            
            // Rest of the program continues with shifted indices
            rom['h08] <= 32'h20C00048;  // ADDI X6, X0, #72
            rom['h09] <= 32'h44C90000;  // STR  X6, [X9, #0]
            rom['h0A] <= 32'h20C00065;  // ADDI X6, X0, #101
            rom['h0B] <= 32'h44C90004;  // STR  X6, [X9, #4]
            rom['h0C] <= 32'h20C0006C;  // ADDI X6, X0, #108
            rom['h0D] <= 32'h44C90008;  // STR  X6, [X9, #8]
            rom['h0E] <= 32'h20C0006C;  // ADDI X6, X0, #108
            rom['h0F] <= 32'h44C9000C;  // STR  X6, [X9, #12]
            rom['h10] <= 32'h20C0006F;  // ADDI X6, X0, #111
